...
internal class Code93  {
    ...
    fun encode(contents: String): BooleanArray {
        ...
        for (i in 0 until length) {
            val indexInString: Int = Code39.ALPHABET_STRING.indexOf(actualContents[i])
            if (indexInString < 0) {
        ...
        toIntArray(Code39.ASTERISK_ENCODING, widths)
        ...
        for (i in 0 until length) {
            val indexInString: Int = Code39.ALPHABET_STRING.indexOf(actualContents[i])
            toIntArray(Code39.CHARACTER_ENCODINGS.get(indexInString), widths)
